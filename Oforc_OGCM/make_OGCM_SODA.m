%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 根据本地SODA月平均数据生成边界场和初始场
% 由于SODA为月平均数据，当itolap_p=2时会出错，程序中将其重赋值为1
% 2018.7.5
...
%  Update     21 - April-2010 by Yu Liu (NUIST)  for HYCOM
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif strcmp(OGCM,'ECCO')
  ...
  %url = 'http://apdrc.soest.hawaii.edu:80/dods/public_data/Model_output/HYCOM/global/'
  %
else
...
if makeclim==1 | makebry==1
  ...
  for Y=Ymin:Ymax
	...
	interp_OGCM(OGCM_dir,OGCM_prefix,Yp,Mp,Roa,interp_method,...
		    lonU,latU,lonV,latV,lonT,latT,Z,tndx_OGCM(aa1),...
